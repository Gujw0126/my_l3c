...
def train_one_epoch(net, optimizer, lr_scheduler, epoch, iteration_before, criterion, dataloader, logger, summery_writer, savepath_train,decay):
...
def set_args(argv):
    ...
    parser.add_argument("--l3cpath",type=str)
...
def main(argv):
    ...
    #net.initialize()
    if args.pretrained:
        if args.l3cpath is None:
            raise RuntimeError("no l3cpath provided")
        l3c_checkpoint = torch.load(args.l3cpath, map_location=device)
        pretrained_state_dict = l3c_checkpoint["model_state_dict"]
        test_state_dict = net.state_dict()
        no_need_train = []
        #meanshift, encoders and decoders(first two layers)
        for key in pretrained_state_dict.keys():
            if key.startswith("meanshift") or key.startswith("head") or key.startswith("encoders"):
                test_state_dict[key] = pretrained_state_dict[key]
                test_state_dict[key].requires_grad=False
                no_need_train.append(key)
            if key.startswith("decoders"):
                if key.split(".")[1]=="0" or key.split(".")[1]=="1" :
                    test_state_dict[key] = pretrained_state_dict[key]
                    test_state_dict[key].requires_grad = False
                    no_need_train.append(key)
        net.load_state_dict(state_dict=test_state_dict)
        all_parameters = {
            n
            for n,p in net.named_parameters()
        }
        no_need_train_parameters = {
            n
            for n, p in net.named_parameters()
            if n in no_need_train
        }
        need_train_parameters = all_parameters ^ no_need_train_parameters
        inter_params = need_train_parameters & no_need_train_parameters
        union_params = need_train_parameters | no_need_train_parameters
        
        params_dict = dict(net.named_parameters())
        assert len(inter_params) == 0
        assert len(union_params) - len(params_dict.keys()) == 0
        optimizer = optim.RMSprop(
            (params_dict[n] for n in sorted(need_train_parameters)),
...
if __name__=="__main__":
    debug=["-d", "/mnt/data3/jingwengu/dataset/openimage", "-e", "30" ,"--cuda", "--savepath", "/mnt/data3/jingwengu/MLC/l3c_pre","--checkpoint","/mnt/data3/jingwengu/MLC/l3c_pre/iteration/checkpoint_train_224000.pth.tar","--pretrained","--l3cpath","/mnt/data3/jingwengu/my_l3c2/iteration/checkpoint_train_700000.pth.tar"]
    main(debug)